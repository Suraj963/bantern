...
// 1. Define your testimonial data (No change)
const testimonialData = [
  ...
  },
  {
    ...
    avatar: "https://images.unsplash.com/photo-1506794778202-cad84cf4F1d?q=80&w=256&auto=format&fit=crop",
...
// 3. Testimonial Card Sub-Component (No change)
const TestimonialCard = ({ testimonial }) => (
  <motion.div
    ...
    "
  >
    <h3 className="text-lg sm:text-xl font-semibold mb-3 text-foreground">{testimonial.heading}</h3>
    <p className="text-xs sm:text-sm text-muted-foreground mb-4 flex-grow">{testimonial.text}</p>
    <div className="flex items-center gap-3 pt-4 border-t border-border/50">
      ...
      />
      <div>
        <h4 className="text-sm sm:text-base text-foreground font-medium">{testimonial.name}</h4>
...
const Testimonials = () => {
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel(
    ...
    [Autoplay({ 
      delay: 4000,
      stopOnInteraction: true,  // Stops on drag (good)
      stopOnMouseEnter: true, // Stops on hover (good)
      playOnInit: false,
    })]
  );

  // --- UPDATED: Logic to manually start and restart autoplay ---
  useEffect(() => {
    if (!emblaApi) return;
    const autoplay = emblaApi.plugins().autoplay;
    if (!autoplay) return;

    // 1. Start autoplay when it scrolls into view
    if (inView) {
      autoplay.play();
    }

    // 2. This function restarts autoplay
    const restartAutoplay = () => {
      autoplay.play();
    };

    // 3. Add listeners to restart autoplay
    // After a drag ends, restart
    emblaApi.on('dragEnd', restartAutoplay);
    // After a pointer (touch/mouse) up, restart
    emblaApi.on('pointerUp', restartAutoplay);
    // When the mouse leaves, restart
  ...
  return (
    ...
    >
      ...
      {/* === SCROLLING CARD CONTAINER (No change) --- */}
      <motion.div 
        variants={itemVariants}
