    ...
    <main className="min-h-screen flex items-center justify-center py-20 bg-background text-foreground px-4">
      <section className="w-full max-w-2xl bg-card p-6 rounded-lg border border-border shadow-sm">
        <h1 className="text-2xl font-bold mb-2">
          Book your free 15-minute business audit
        </h1>
        <p className="text-sm text-muted-foreground mb-6">
          Quick audit focused on customer experience and lead conversion. We'll
          review your site and give 3 immediate fixes.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="flex flex-col">
              ...
              <input
                name="name"
                required
                className="px-3 py-2 border border-border rounded-md bg-transparent"
          ...
          <label className="flex flex-col">
            ...
            >
              ...
              <option value="local-service">
                Local Service (Plumber, Salon, etc.)
              </option>
              <option value="restaurant">Restaurant / Food</option>
              <option value="tech-saas">Tech / SaaS</option>
              <option value="professional-service">
                Professional Service (Law, etc.)
              </option>
          ...
          <label className="flex flex-col">
            <span className="text-xs text-muted-foreground mb-1">
              Preferred audit time (approx)
            </span>
            <input
              name="preferred_time"
              className="px-3 py-2 border border-border rounded-md bg-transparent"
              placeholder="Mon-Fri, 10am - 2pm IST"
            />
          </label>
          <label className="flex flex-col">
            <span className="text-xs text-muted-foreground mb-1">
            ...
            <textarea
              ...
              rows="4"
              className="px-3 py-2 border border-border rounded-md bg-transparent"
              placeholder="More leads, better online booking, e-commerce setup..."
            ></textarea>
          </label>
            ...
            <div className="text-sm text-muted-foreground">
              {status === "success" && (
                <span className="text-green-500">
                  Booked — check your email shortly.
                </span>
              )}
              {status === "error" && (
          ...
          <p className="text-xs text-muted-foreground mt-3">
            We respect your privacy. We will only use this info to conduct the
            audit and follow up about our services.
          </p>
        </form>
      </section>
    </main>
